...
sigularity_path_temp=`which singularity`
if [ "$sigularity_path_temp" != "" ]; then singularity_path="$singularity_path_temp"; fi
...
${singularity_path} run "$sdir"/../singularity_image/seqkit.sif seqkit fx2tab ${db}.maskadaptors|awk -F"\t" '{split($1,array,".");print array[1];}'|
